...
app = FastAPI(title="Hệ Thống Gợi Ý Bài Tập - Tích hợp GenAI Chấm Code")
...
def grade_and_submit_result(request: MockGradeRequest, x_user_role: str = Header(None, description="Bắt buộc nhập 'student'")):
    ...
    try:
        conn = get_db_connection()
        # 1. Lấy thông tin bài tập để đưa cho AI
        ...
        # 2. Xử lý logic chấm điểm với Google Gemini AI
        ...
        if gemini_api_key and len(code_sinh_vien) > 15:
            ...
            # Sử dụng model Gemini 1.5 Pro hoặc Flash để xử lý code
            model = genai.GenerativeModel('gemini-1.5-flash')
            prompt = f"""Bạn là giảng viên chấm thi lập trình nghiêm khắc nhưng công tâm.
            Hãy chấm điểm đoạn code của sinh viên dưới đây.
            ...
            HÃY PHÂN TÍCH VÀ TRẢ VỀ ĐÚNG ĐỊNH DẠNG JSON SAU (Không Markdown, không text thừa):
            {{
                "score": <Điểm số từ 0.0 đến 10.0, kiểu float>,
                "criteria_eval": "<Nhận xét ngắn gọn xem code có đạt các tiêu chí thuật toán không>",
                "strengths": "<Chỉ ra phần code viết đúng, tối ưu hoặc logic tốt nhất>",
                "weaknesses": "<Chỉ ra lỗi sai, bug, hoặc phần code tệ nhất cần sửa chữa>"
            }}
            """
            try:
                ...
                feedback = {"criteria_eval": "Lỗi phân tích AI.", "strengths": "Tạm ổn.", "weaknesses": "Cần cải thiện."}
        else:
            # Fallback nếu không có API Key hoặc sinh viên nộp code rỗng
            if len(code_sinh_vien) <= 15:
                final_grade = 0.0
                feedback = {"criteria_eval": "Code quá ngắn hoặc không hợp lệ.", "strengths": "Không có", "weaknesses": "Chưa nộp code thực sự."}
            else:
                final_grade = round(random.uniform(4.0, 9.0), 1)
                feedback = {"criteria_eval": "Đã chấm tự động.", "strengths": "Cấu trúc cơ bản", "weaknesses": "Chưa tối ưu"}

        # 3. Lưu điểm vào cơ sở dữ liệu
        query_upsert = text("""
...
def get_recommendations_cbf(request: RecommendRequest, x_user_role: str = Header(None, description="Bắt buộc nhập 'student'")):
    ...
    # SỬA LỖI ẢNH 1: Dùng Regex chứa cả cụm từ viết tắt để lọc đúng môn trong SQL
...
def export_student_progress(student_id: int, x_api_key: str = Header(None, description="Khóa bảo mật")):
    if x_api_key != "KHOA_BIMAT_CUA_NHOM_AI": raise HTTPException(status_code=401, detail="Cảnh báo: Bạn không có quyền!")
    try:
        conn = get_db_connection()
